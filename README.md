...
- 胜平负与比分模型都在开赛前时间验证中比较多种时间尺度；近似同分时优先近期，当前均选择 1.5 年半衰期
- 每场比赛先构造 47 项赛前变量；胜平负使用验证选中的 `core-25`，比分使用 `form-41`，避免无效变量增加噪声
...
- 晋级评分使用同一套通用权重：赛前实力 50%、本届即时状态 44%、历史分类器 5%、赛前环境与疲劳 1%
...
- 小组赛校准器以前 48 场拟合、后 24 场按时间验证，再用全部 72 场重拟合；当前淘汰赛比分融合 25% 校准结果
- 当前 28 场淘汰赛严格精确比分命中 `8/28（28.6%）`，每队平均绝对误差 `0.57 球`
- 单队进球误差不超过 1 球为 `51/56（91.1%）`，双方都不超过 1 球为 `24/28（85.7%）`；两者均不等同于精确比分命中率
...
模型的数据来源、完整权重、公式和自主修改位置见根目录 `预测模型说明与自主修改指南.txt`。
...
- 在近似同分容差内优先更简单的胜负变量集，降低偶然验证波动造成的过拟合
- 分别训练多组泊松梯度提升进球模型，并保留直接进球分类器作为受控对照
- 只用本届开赛前的时间外验证，为胜平负和比分模型分别选择变量集、模型结构与时间半衰期
