...
- 预测模式使用纵向“世界杯冠军之路”画布，上下半区向中央决赛汇聚，冠军路径以金色高亮
- 点击任意分支或对阵可打开比赛预测；左键点击国旗可更换球队
- 32 强替换严格限制在对应小组来源且不可重复，后续轮次只能选择对应上游比赛的参赛队
- 48 面国旗保存在 `public/assets/flags/`，避免外部热链在电脑端或 GitHub Pages 加载失败
...
- `src/data/marketValues.json`：2026-06-23 球队总身价、已核验球员身价与伤停状态快照
...
### 当前状态模型（2026-07-13 快照）

当前版本不再只依赖历史国家队比赛。预测画像由以下证据共同构成：

- 历史样本严格按时间切为 70% 训练、15% 调参、15% 最终评估；每个回测切分都以当时的截止日计算时间衰减
- 胜平负与比分模型都比较多种时间尺度；当前历史基线均选择 5 年半衰期，本届时效性另外由当前赛事状态与最近 3 场信号提供
- 每场比赛先构造 47 项赛前变量；胜平负使用验证选中的 `full-47`，比分使用 `form-41`
- 比分模型在普通泊松 HGB、强正则泊松 HGB、近期灵敏泊松 HGB 和直接进球分类器之间验证，当前选择 `poisson-hgb-regularized`
- 比分分布比较独立泊松、Dixon-Coles 和双变量泊松，当前选择 Dixon-Coles 低比分修正（`rho=-0.05`）
- 胜平负概率使用温度校准，并同时报告准确率、Log loss、Brier score 和 RPS
- 世界杯正赛权重最高，友谊赛权重最低
- 纳入截至 `2026-07-13` 已结束的完整 100 场比赛：72 场小组赛 + 28 场淘汰赛
- 本届世界杯状态在球队综合画像中的占比会随场次上升，5 场及以上达到 60%
- 2026-06-28 起逐场保存赛前快照，晋级者回放命中 24/28（85.7%），胜/平/负命中 18/28（64.3%）
- 32 强和 16 强的 24 场用于选择一组全局权重，命中 20/24（83.3%）；时间更晚的 4 场四分之一决赛不参与自动选参，命中 4/4
- 晋级评分使用同一套通用权重：赛前实力 50%、本届即时状态 39%、历史分类器 10%、赛前环境与疲劳 1%
- 历史 CSV 严格截断到本届开赛前；其中开赛日起的 72 行均为无比分赛程，没有进入训练。模型选型完成后，再用全部赛前历史赛果重新拟合最终分类器
- 环境项只使用赛前可知的场馆、海拔、屋顶类型、七月气候基线、休息天数和旅行距离，不使用赛后观测天气
- 纳入当前 48 支参赛队的公开阵容总身价快照
- 纳入当前预测名单的位置覆盖、关键球员、预计首发、阵容可用性、关键伤停、球员角色适配、战术适配、角色默契和教练调整能力
- 单场淘汰赛根据具体对手重新计算，不直接照搬球队总排名
- 胜率极接近时使用综合实力与模型置信度稳定结果，避免小样本造成机械翻转
- 比分模型分别估计双方预期进球，并融合历史模型、本届赛事节奏、整届攻防均值、最近 3 场攻防和小组赛时效校准器
- 预测比分包含 90 分钟与可能发生的 30 分钟加时；点球大战进球不计入比赛比分
- 小组赛校准器以前 48 场拟合、后 24 场按时间验证，再用全部 72 场重拟合；当前淘汰赛比分融合 50% 校准结果
- 当前 28 场淘汰赛严格精确比分命中 `11/28（39.3%）`，每队平均绝对误差 `0.54 球`
- 单队进球误差不超过 1 球为 `49/56（87.5%）`，双方都不超过 1 球为 `22/28（78.6%）`；两者均不等同于精确比分命中率

完整赛果位于 `data/current/world_cup_2026_results.json`，场馆环境基线位于 `data/current/world_cup_2026_venues.json`，人员与战术快照位于 `data/current/world_cup_2026_state.json`。人工评分属于分析估计，不是官方评级；伤停或首发信息变化后，应先更新快照再重新训练。

> 85.7% 是本届 28 场淘汰赛的晋级方向顺序回放结果，不是比分准确率，也不是对未来赛事的保证。四分之一决赛没有进入自动参数搜索，但人工迭代已经查看过其结果，因此现在应视为 4 场诊断时间片，而不是完全未触碰的外部测试。

> 足球精确比分属于低基准率事件。项目不会为了达到预设数字而把赛果泄漏进特征、为某场比赛增加专属规则，或把“误差不超过 1 球”写成精确比分准确率。

模型的数据来源、完整权重、公式和自主修改位置见根目录 `预测模型说明与自主修改指南.txt`；方法研究来源另存于 `data/current/model_research_sources.json`。
...
- 读取历史比赛结果
- 构建 47 项纯赛前变量：Elo、5/10/20 场状态、进失球趋势与波动、零封率、被零封率、对手强度、休息天数、30/90 天赛程负荷、比赛类型与时间衰减
- 使用样本权重训练：世界杯正赛最高，洲际杯次之，预选赛和友谊赛较低，并让近期比赛获得更高权重
- 训练 `HistGradientBoostingClassifier`
- 同时训练 Logistic Regression 作为基线对照
- 在调参准确率接近最优的候选中，优先更低的 RPS 与 Log loss，再考虑模型复杂度
- 分别训练多组泊松梯度提升进球模型，并保留直接进球分类器作为受控对照
- 只用本届开赛前的中间时间调参段选择变量集、模型结构、时间半衰期、概率温度和比分分布；最后 15% 仅用于最终报告
- 比较独立泊松、Dixon-Coles 低比分修正和双变量泊松共同进球分布
- 用 72 场小组赛建立带正则的泊松时效校准器，并以 48/24 时间切分选择正则强度
- 叠加当前世界杯赛果、阵容可用性与战术情境
- 对 28 场淘汰赛做逐场赛前回放，并把晋级者与胜/平/负分开评估
- 分开审计精确比分、每队平均误差、单队误差不超过 1 球和双方均不超过 1 球四项指标
- 根据已知赛程计算场馆、七月温湿度基线、海拔、休息和旅行负担
- 输出 `src/data/modelPredictions.ts` 和本地审计文件 `outputs/current_model_profiles.json`

训练后可运行完整性检查：

```powershell
D:\AnacondaTools\anac\envs\wc2026\python.exe scripts\audit_model_integrity.py
```
...
- 美国海军蓝：页面底色、数据层级和专业模式
- 加拿大枫叶红：主操作、当前板块和关键提醒
- 墨西哥绿：可用状态、成功反馈和新手引导
- 白色：信息分隔和浅色模式留白
- 金色：仅保留给冠军路径和顶级强队，避免重点信息全部使用同一种强调色
