...
- 历史比赛采用 3 年半衰期的指数时间衰减，长期最低权重为 1%，最近 12 个月另有平滑加成
- 世界杯正赛权重最高，友谊赛权重最低
...
- 小组赛校准器以前 48 场拟合、后 24 场按时间验证，再用全部 72 场重拟合；淘汰赛比分只融合 25% 校准结果
- 当前 28 场淘汰赛严格精确比分命中 `11/28（39.3%）`，每队平均绝对误差 `0.59 球`
- 单队进球误差不超过 1 球为 `47/56（83.9%）`，双方都不超过 1 球为 `21/28（75.0%）`；两者均不等同于精确比分命中率
