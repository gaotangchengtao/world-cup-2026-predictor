...
- 胜平负分类器采用 3 年半衰期；比分进球模型从 1、1.5、2、3、5 年五个候选中按赛前时间验证自动选择 1.5 年半衰期
- 世界杯正赛权重最高，友谊赛权重最低
- 纳入截至 `2026-07-13` 已结束的完整 100 场比赛：72 场小组赛 + 28 场淘汰赛
- 本届世界杯状态在球队综合画像中的占比会随场次上升，5 场及以上达到 60%
- 2026-06-28 起逐场保存赛前快照，晋级者回放命中 24/28（85.7%），胜/平/负命中 17/28（60.7%）
...
- 单队进球误差不超过 1 球为 `50/56（89.3%）`，双方都不超过 1 球为 `23/28（82.1%）`；两者均不等同于精确比分命中率
...
- 选择验证准确率更好的模型
- 分别训练两个泊松梯度提升进球模型，生成双方预期进球与最可能比分
- 在五个历史时间衰减候选中，只用本届开赛前的时间外验证选择比分模型半衰期
