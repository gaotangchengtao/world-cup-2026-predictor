...
- 32 强和 16 强的 24 场用于选择一组全局权重，命中 20/24（83.3%）；时间更晚的 4 场四分之一决赛完全留出，命中 4/4
...
- 比分模型分别估计双方预期进球，并融合历史模型、本届赛事节奏、整届攻防均值、最近 3 场攻防和赛前环境
- 预测比分包含 90 分钟与可能发生的 30 分钟加时；点球大战进球不计入比赛比分
- 当前 28 场淘汰赛严格精确比分命中 `10/28（35.7%）`，每队平均绝对误差 `0.63 球`
- 单队进球误差不超过 1 球为 `47/56（83.9%）`，双方都不超过 1 球为 `22/28（78.6%）`；两者均不等同于精确比分命中率
...
> 85.7% 是本届 28 场淘汰赛的晋级方向顺序回放结果，不是比分准确率，也不是对未来赛事的保证。四分之一决赛独立留出集只有 4 场，必须和 83.3% 的开发回放、60.7% 的胜平负指标一起阅读。
