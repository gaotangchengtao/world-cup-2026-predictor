...
export const TopPlayers = ({ players, teams, onSelectPlayer }: TopPlayersProps) => {
    ...
    <article className="mobile-snap-card glass-panel rounded-lg p-3.5 sm:p-4">
      ...
      <div className="mt-4 grid gap-2">
        {rows.map((player, index) => {
          ...
          return (
            <button
              className="flex items-center gap-2.5 rounded-lg border border-white/10 bg-white/5 p-2.5 text-left transition hover:border-trophy-500 light:border-slate-900/10 light:bg-white sm:gap-3 sm:p-3"
              key={player.playerId}
  ...
  return (
    <section className="mobile-snap-grid grid gap-3 sm:gap-4 xl:grid-cols-2">
